...
export default {
  ...
  setup() {
    ...
    const can_save = computed(() => !is_busy.value && is_dirty.value && !has_conflict.value && Boolean(task_state.value.draft.original_description.trim()));
    const update_field = (key, value) => store.commit('tasks/set_field', { key, value });
    const allow_leave = () => !is_busy.value && (!is_dirty.value || window.confirm('Есть несохранённые изменения. Покинуть страницу и потерять их?'));
    ...
    const warn_before_unload = event => {
      if (is_dirty.value || is_busy.value) { event.preventDefault(); event.returnValue = ''; }
    };
    ...
    const reload_task = () => {
      if (is_dirty.value && !window.confirm('Загрузить сохранённую версию? Несохранённый текст будет потерян.')) return;
      store.dispatch('tasks/load_task', route.params.id);
    };
    const confirm_task = () => store.dispatch('tasks/confirm_task');
    return { task_state, is_busy, is_dirty, can_confirm, has_conflict, can_save, can_publish, is_published, is_publication_current, publish_task,
